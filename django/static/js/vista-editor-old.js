...
$(document).ready(function () {
    ...
    $('#btn_crear_nuevo_proyecto').click(function (e) {
        ...
        let url = 'http://127.0.0.1:8000/servicios/videos/editor/crear_nuevo_proyecto/';
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.proyecto && data.proyecto.id && data.proyecto.nombre) {
                    ...
                    window.location.href = `http://127.0.0.1:8000/servicios/videos/editor/${data.proyecto.referencia_proyecto}`;
                } else {
                    Swal.fire({
...
function eliminar_proyecto(referencia_proyecto) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            let url = 'http://127.0.0.1:8000/servicios/videos/editor/eliminar_proyecto/' + referencia_proyecto + '/';
            let csrfToken = $('[name=csrfmiddlewaretoken]').val();
            $.ajax({
                type: 'POST',
                url: url,
...
function activar_dropzone() {
    if (document.querySelector('.dropzone')) {
        let url = "http://127.0.0.1:8000/servicios/videos/editor/subir_archivos/";
...
function subir_archivos() {
    ...
    let url_subir_archivos = "http://127.0.0.1:8000/servicios/videos/editor/subir_archivos/";
    $.ajax({
...
function obtener_archivos_del_proyecto(referencia_proyecto) {
    let url_obtener_archivos = "http://127.0.0.1:8000/servicios/videos/editor/proyecto/obtener_archivos/";
...
function listar_archivos(archivos) {
    let url_base = "http://127.0.0.1:8000/media/";
    if (archivos.length > 0) {
...
async function crear_capturas(video_index, id_archivo, proyecto_ref) {
    try {
        ...
        const url = 'http://127.0.0.1:8000/servicios/videos/editor/crear_capturas/';
...
async function mostrar_capturas(capturas, archivo_id, proyecto_ref, indexVideo) {
    ...
    console.log('array_capturas', capturas);
    console.log('array_videos_a_manipular', videos_a_manipular);
    console.log('indexVideo', indexVideo);
    console.log('videoActual', videos_a_manipular[indexVideo]);
    ...
    galeriaContainer.classList.add('galeria-capturas');
    ...
    galeriaContainer.style.flexDirection = 'row';
    galeriaContainer.style.justifyContent = 'flex-start';
    galeriaContainer.style.alignItems = 'center';
    galeriaContainer.style.maxWidth = (anchoImagen * capturas.length) + 'px';  // Ajustar el ancho máximo

    // Contador para llevar el control de imágenes cargadas
    let imagenesCargadas = 0;

    // Agregar la galería al DOM
...
function eliminar_archivo(archivo_id) {
    let url_eliminar_archivos = "http://127.0.0.1:8000/servicios/videos/editor/proyecto/eliminar_archivo/";
